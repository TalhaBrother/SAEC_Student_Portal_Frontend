...
const Field = ({ label, children, hint }) => (
  <div className="flex flex-col w-full">
    ...
    {children}
    {hint && <p className="text-neutral-400 text-xs mt-1">{hint}</p>}
  </div>
);
...
const primaryBtnClass =
  "bg-primary hover:bg-quinary disabled:opacity-50 text-white font-medium py-3 px-6 rounded-xl transition-all duration-300 shadow-md transform active:scale-[0.98] cursor-pointer text-sm font-semibold tracking-wide uppercase";
const secondaryBtnClass =
  "bg-surface border border-neutral-300 hover:border-primary disabled:opacity-50 text-quinary font-medium py-3 px-6 rounded-xl transition-all duration-300 cursor-pointer text-sm font-semibold tracking-wide uppercase";

const StatCard = ({ label, value, accent }) => (
  <div className="bg-surface rounded-2xl border border-neutral-200 shadow-sm p-5 flex flex-col gap-1">
    <span className="text-xs uppercase tracking-wider text-neutral-500 font-semibold">{label}</span>
    <span className={`text-2xl font-bold ${accent || 'text-quinary'}`}>{value}</span>
  </div>
);

const EmptyState = ({ text }) => (
  <div className="bg-surface rounded-2xl border border-dashed border-neutral-300 p-10 text-center text-neutral-400 text-sm">
    {text}
  </div>
);

const TableShell = ({ headers, children }) => (
  <div className="bg-surface rounded-2xl border border-neutral-200 shadow-sm overflow-x-auto">
      ...
      <thead>
        <tr className="border-b border-neutral-100 bg-neutral-50/60">
          {headers.map((h) => (
            <th
              key={h}
              className="text-left px-4 py-3 text-xs uppercase tracking-wider text-neutral-500 font-semibold whitespace-nowrap"
            >
              {h}
            </th>
          ))}
...
const PeriodToggle = ({ value, onChange }) => (
  <div className="inline-flex rounded-xl border border-neutral-200 bg-surface p-1 shadow-sm">
    {PERIODS.map((p) => (
      <button
        key={p.key}
        type="button"
...
const ModeToggle = ({ value, onChange }) => (
  <div className="inline-flex rounded-xl border border-neutral-200 bg-surface p-1 shadow-sm mb-6">
    {MODES.map((m) => (
      <button
        key={m.key}
        type="button"
...
const StudentSummaryCard = ({ student }) => (
  <div className="bg-surface rounded-2xl border border-neutral-200 shadow-sm p-6 max-w-3xl grid grid-cols-2 md:grid-cols-4 gap-4">
    <div>
      <p className="text-xs uppercase tracking-wider text-neutral-500 font-semibold mb-1">GR No</p>
      <p className="font-medium">{student.gr_no}</p>
    </div>
    <div>
      <p className="text-xs uppercase tracking-wider text-neutral-500 font-semibold mb-1">Name</p>
      <p className="font-medium">{student.full_name}</p>
    </div>
    <div>
      <p className="text-xs uppercase tracking-wider text-neutral-500 font-semibold mb-1">Father Name</p>
      <p className="font-medium">{student.father_name}</p>
    </div>
    <div>
      <p className="text-xs uppercase tracking-wider text-neutral-500 font-semibold mb-1">Class</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <div className="flex flex-col max-w-sm">
              ...
              {checkingExisting && (
                <p className="text-xs text-neutral-400 mt-1">Checking for an existing structure...</p>
              )}
        ...
        <div className="space-y-4">
          ...
          <div className="bg-surface rounded-2xl border border-neutral-200 shadow-sm p-6 max-w-3xl">
            {checkingExisting ? (
              ...
              <>
                <p className={`text-xs mb-4 ${overrideId ? 'text-primary' : 'text-neutral-400'}`}>
                  {overrideId
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-sm font-semibold tracking-wide transition-colors cursor-pointer ${
              activeTab === tab.key
                ...
                : 'bg-surface text-neutral-500 border border-neutral-200 hover:text-quinary hover:border-primary'
            }`}
          >
            {tab.label}
          </button>
    ...
    <div className="space-y-6">
      ...
      {data && period === 'monthly' && (
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 max-w-4xl">
          ...
          <StatCard label="Paid" value={data.paid_count} accent="text-success" />
          <StatCard label="Unpaid" value={data.unpaid_count} accent="text-warning" />
          <StatCard label="Amount Collected" value={formatMoney(data.amount_collected)} accent="text-success" />
          <StatCard label="Amount Pending" value={formatMoney(data.amount_pending)} accent="text-warning" />
        </div>
      )}
    ...
    <div className="space-y-6">
      ...
      {rows.length > 0 && (
        <TableShell headers={['Challan No', 'GR No', 'Student', 'Class', 'Month/Year', 'Amount', 'Status', 'Paid At']}>
          {rows.map((row) => (
            <tr key={row.id} className="border-b border-neutral-50 last:border-0 hover:bg-neutral-50/50">
              ...
              <td className="px-4 py-3 text-neutral-500">{formatDateTime(row.paid_at)}</td>
            </tr>
          ))}
        </TableShell>
      )}
...
const StudentSearchTab = ({ token }) => {
  ...
  const toggleStatus = async (voucher) => {
    ...
    const confirmResult = await Swal.fire({
      ...
      confirmButtonColor: 'var(--quinary)',
            ...
            <TableShell headers={['Challan No', 'Month/Year', 'Due Date', 'Status', 'Paid At', 'Action']}>
              {student.fee_vouchers.map((v) => (
                <tr key={v.id} className="border-b border-neutral-50 last:border-0 hover:bg-neutral-50/50">
                  ...
                  <td className="px-4 py-3 text-neutral-500">{formatDateTime(v.paid_at)}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      {v.status === 'PAID' ? (
                        <span
                          className="text-xs font-semibold uppercase tracking-wide text-neutral-400 cursor-not-allowed"
                        ...
                        <button
                          ...
                          className="text-xs font-semibold uppercase tracking-wide text-primary hover:text-quinary disabled:opacity-50 cursor-pointer"
                        >
...
const Fees = () => {
  ...
  return (
    <div className="p-6 bg-secondary text-quinary min-h-screen font-sans">
      <div className="text-3xl font-bold tracking-tight mb-2 text-quinary">
        Fees
      </div>
      <p className="text-neutral-500 text-sm mb-6">
